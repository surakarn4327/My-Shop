  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="pri-btn" onClick={save} disabled={loading}>
...
function AddTab({ type, categories, onSaved }) {
  ...
  useEffect(() => {
    if (cats.length && !catId) {
      setCatId(cats[0].id);
      setCatName(cats[0].name);
    }
  }, [categories]);

  async function submit() {
    if (!amount || Number(amount) <= 0) {
      setErr("กรุณาใส่จำนวนเงิน");
      return;
    }
    if (!catId) {
  ...
  return (
    <div className="tab-body">
      {ok && (
        <div className={`banner ${isInc ? "b-inc" : "b-exp"}`}>
          ✅ บันทึกสำเร็จ!
        </div>
      )}
      {err && <div className="err-box">{err}</div>}
      ...
      <div className="fg">
        ...
        <div className="amt-wrap">
          <span className={`sign ${isInc ? "s-inc" : "s-exp"}`}>
            {isInc ? "+" : "−"}
          </span>
          <input
            className="amt-inp"
      ...
      <div className="fg">
        <label>
          หมายเหตุ <span className="opt">(ไม่บังคับ)</span>
        </label>
        <input
      ...
      <button
        className={`sub-btn ${isInc ? "sb-inc" : "sb-exp"}`}
        onClick={submit}
        disabled={loading}
      >
        {loading
...
function HistoryTab({ refreshKey }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setTxns(await getTransactions(filterDate ? { date: filterDate } : {}));
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  ...
  return (
    <div className="tab-body">
      ...
      <div className="sum-row">
        <div className="sc inc">
          <div className="slb">รายรับ</div>
          <div className="svl">฿{fmt(totInc)}</div>
        </div>
        <div className="sc exp">
          <div className="slb">รายจ่าย</div>
          <div className="svl">฿{fmt(totExp)}</div>
        </div>
        <div className={`sc ${profit >= 0 ? "prf" : "los"}`}>
          <div className="slb">กำไร</div>
          <div className="svl">฿{fmt(profit)}</div>
        </div>
      </div>
      {loading ? (
        ...
        txns.map((t) => (
          <div key={t.id} className={`txn ${t.type}`}>
            ...
            <div
              className={`txn-amt ${t.type === "income" ? "ta-inc" : "ta-exp"}`}
            >
              {t.type === "income" ? "+" : "−"}฿{fmt(t.amount)}
            </div>
            <button className="del" onClick={() => del(t.id)}>
              🗑
            </button>
          </div>
        ))
...
function SummaryTab({ refreshKey }) {
  ...
  const load = useCallback(async () => {
    try {
      setData(await getSummary(month));
    } catch (e) {
      console.error(e);
    }
  }, [month, refreshKey]);
  ...
  return (
    <div className="tab-body">
      ...
      <div className="sum-row">
        <div className="sc inc">
          <div className="slb">รายรับรวม</div>
          <div className="svl">฿{fmt(data.totInc)}</div>
        </div>
        <div className="sc exp">
          <div className="slb">รายจ่ายรวม</div>
          <div className="svl">฿{fmt(data.totExp)}</div>
        </div>
        <div className={`sc ${data.totInc - data.totExp >= 0 ? "prf" : "los"}`}>
          <div className="slb">กำไรสุทธิ</div>
          <div className="svl">฿{fmt(data.totInc - data.totExp)}</div>
        </div>
      </div>
      {expCats.length > 0 && (
          ...
          {expCats.map((c) => (
            <div key={c.categoryName} className="bar-row">
              ...
              <div className="bar-trk">
                <div
                  className="bar-fil ef"
                  style={{ width: `${Math.round((c.total / maxExp) * 100)}%` }}
                />
              </div>
        ...
        <div className="chart-box">
          ...
          {incCats.map((c) => (
            <div key={c.categoryName} className="bar-row">
              ...
              <div className="bar-trk">
                <div
                  className="bar-fil if"
                  style={{
                    width: `${Math.round((c.total / data.totInc) * 100)}%`,
                  }}
                />
              </div>
              <div className="bar-val">฿{fmt(c.total)}</div>
      ...
      {data.byCategory.length === 0 && (
        <div className="empty">ยังไม่มีข้อมูลในเดือนนี้</div>
      )}
    </div>
  );
}
      ...
      {categories.map((c) => (
        <div key={c.id} className="cat-row">
          ...
          <div className="cat-inf">
            ...
            <div className="cat-tp">{typeLabel[c.type]}</div>
          </div>
          {!c.isDefault && (
            <button className="del" onClick={() => del(c)}>
              🗑
            </button>
          )}
        </div>
...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        {tab === "expense" && (
          <AddTab type="expense" categories={categories} onSaved={onSaved} />
        )}
        {tab === "income" && (
          <AddTab type="income" categories={categories} onSaved={onSaved} />
        )}
        {tab === "history" && <HistoryTab refreshKey={refreshKey} />}
        {tab === "summary" && <SummaryTab refreshKey={refreshKey} />}
        {tab === "categories" && (
          <CategoriesTab categories={categories} onReload={loadCats} />
        )}
      </main>
      <nav className="bnav">
        {TABS.map((t) => (
